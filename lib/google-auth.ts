...
class GoogleAuthService {
  ...
  async signInWithPopup(): Promise<string> {
    ...
    // Try direct OAuth flow instead of Google Identity Services
    return new Promise((resolve, reject) => {
      ...
      console.log('[GOOGLE-AUTH-SERVICE] Opening OAuth popup:', authUrl);
  ...
  async signInWithCredential(token: string): Promise<any> {
    ...
    // Token is already the access_token from backend, just return it
    return {
      access_token: token,
      token_type: 'bearer'
    };
